...
class Fork:
    def __init__(self, index: int):
        ...
        self.priority: int = index  # Added priority based on the index
...
class Philosopher(threading.Thread):
    ...
    def think(self):
        with self.left_fork(self.index), self.right_fork(self.index):
            # Release both forks before sleeping
            pass

        time.sleep(3 + random.random() * 3)

    def eat(self):
        small_fork, big_fork = sorted([self.left_fork, self.right_fork], key=lambda x: x.index)

        with small_fork(self.index):
            with big_fork(self.index):
